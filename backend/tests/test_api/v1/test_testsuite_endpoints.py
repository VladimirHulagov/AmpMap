...
class TestSuiteEndpoints:
    ...
    def test_list(self, api_client, authorized_superuser, test_suite_factory, project):
        ...
        response = api_client.send_request(self.view_name_list, query_params={'project': project.id}).json()['results']
    ...
    @pytest.mark.parametrize('fixture_name', ['test_suite', 'test_suite_with_cases'])
    def test_partial_update(self, request, api_client, authorized_superuser, fixture_name):
        test_suite = request.getfixturevalue(fixture_name)
    ...
    def test_update(self, request, api_client, authorized_superuser, expected_status, fixture_name, project):
        new_name = 'new_expected_test_case_name'
        test_suite = request.getfixturevalue(fixture_name)
        suite_dict = {
            'id': test_suite.id,
    ...
    def _validate_copied_objects(
        ...
        assert len(src_objects_copied) == len(copied_objects_copied), 'Both validating objects lengths must be equal'
...
class TestSuiteEndpointsQueryParams:
    ...
    def test_suite_filter(self, api_client, authorized_superuser, test_suite_factory, project_factory):
        ...
        for project in projects:
            ...
            actual_dict = response.json()['results']
    ...
    def test_search(self, api_client, authorized_superuser, test_suite_factory, project):
        ...
        expected_output = model_to_dict_via_serializer(
            root_suite,
            TestSuiteMockTreeSerializer,
            fields_to_add={'cases_count': 0, 'total_cases_count': 0, 'estimates': None, 'total_estimates': None},
        )
        inner_dict = model_to_dict_via_serializer(
            inner_suite,
            TestSuiteMockTreeSerializer,
            fields_to_add={'cases_count': 0, 'total_cases_count': 0, 'estimates': None, 'total_estimates': None},
        )
        inner_dict['children'] = model_to_dict_via_serializer(
            expected_suites,
            TestSuiteMockTreeSerializer,
            many=True,
            fields_to_add={'cases_count': 0, 'total_cases_count': 0, 'estimates': None, 'total_estimates': None},
        ...
        actual_data[0]['children'][0]['children'] = sorted(
            actual_data[0]['children'][0]['children'], key=itemgetter('id'),
        )
        assert actual_data == [expected_output], 'Search data does not match expected data.'
        actual_data = api_client.send_request(
            self.view_name_list,
    ...
    def test_search_with_path(self, api_client, authorized_superuser, test_suite_factory, project):
        root_suite = test_suite_factory(project=project)
        inner_suite = test_suite_factory(project=project, parent=root_suite)
        search_name = 'search_name'
        ...
        actual_suite_list = api_client.send_request(
            self.view_name_list,
            expected_status=HTTPStatus.OK,
            request_type=RequestType.GET,
    ...
    def test_case_counting(
        ...
        test_suite_factory, project, test_case_factory,
    ):
        cases = []
        ...
        actual_dict = api_client.send_request(
            self.view_name_list,
            expected_status=HTTPStatus.OK,
            request_type=RequestType.GET,
            query_params={'project': project.id, 'treeview': 'True'},
    ...
    def test_flat_list_with_paths(
        ...
        actual_suite_list.sort(key=lambda suite: suite['id'])
