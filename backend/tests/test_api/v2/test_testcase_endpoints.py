...
from tests.mock_serializers.v2 import TestCaseListMockSerializer, TestCaseMockSerializer, TestMockSerializer
...
class TestCaseEndpoints:
    view_name_detail = 'api:v2:testcase-detail'
    view_name_list = 'api:v2:testcase-list'
    view_name_copy = 'api:v2:testcase-copy'
    view_name_history = 'api:v2:testcase-history'
    view_name_tests = 'api:v2:testcase-tests'
    ...
    def test_list(self, superuser_client, test_case_factory, project):
        ...
        with allure.step('Validate response body'):
            assert response.json_strip() == expected
    ...
    def test_retrieve(self, request, superuser_client, test_case_factory, is_archive):
        allure.dynamic.title(f'Test retrieve {request.node.callspec.id}')
        ...
        ).json_strip(is_paginated=False, as_json=True)
        with allure.step('Validate response body'):
...
class TestCaseWithStepsEndpoints:
    ...
    def test_retrieve(self, superuser_client, test_case_with_steps):
        ...
        with allure.step('Validate response body'):
            assert response.json_strip(is_paginated=False) == expected_dict, 'Expected and actual dict did not match'
    ...
    def test_update(self, superuser_client, test_case_with_steps, field_to_update):
        ...
        update_dict['suite'] = update_dict['suite']['id']
        superuser_client.send_request(
            self.view_name_detail,
            data=update_dict,
...
class TestCaseFilter:
    ...
    def test_filter_by_suite(self, test_case_factory, test_suite_factory, project, superuser_client):
        expected_filtered = 2
        expected_total = 4
        suite_ids = []
        for _ in range(2):
            ...
            for _ in range(expected_filtered):
                test_case_factory(suite=suite, project=project)
        response = superuser_client.send_request(
            self.view_name_list,
            query_params={'project': project.id, 'suite': suite_ids[0]},
        ).json()
        assert expected_filtered == response['count']
        response = superuser_client.send_request(
            self.view_name_list,
            query_params={'project': project.id, 'suite': join_iterable(suite_ids)},
        ).json()
