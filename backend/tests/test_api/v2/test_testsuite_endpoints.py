...
class TestSuiteEndpoints:
    view_name_list = 'api:v2:testsuite-list'
    view_name_detail = 'api:v2:testsuite-detail'
    ...
    def test_copy_suite_with_migrating_steps(self, authorized_superuser_client, project, test_suite_factory):
        ...
        case_list_reverse = 'api:v2:testcase-list'
        case_detail_reverse = 'api:v2:testcase-detail'
        payload = {
            'name': 'Case',
            'attributes': {},
            'attachments': [],
    ...
    def test_suite_union(self, test_suite_factory, test_case_factory, project, authorized_superuser_client):
        roots = []
        child_suites = defaultdict(list)
        for _ in range(3):
            suite = test_suite_factory(project=project)
            child_suite = model_to_dict_via_serializer(
                test_suite_factory(project=project, parent=suite),
                TestSuiteUnionMockSerializer,
                refresh_instances=True,
            )
            child_case = model_to_dict_via_serializer(
                test_case_factory(project=project, suite=suite),
                TestCaseUnionMockSerializer,
                refresh_instances=True,
            )
            child_suites[suite.pk].append(child_suite)
            child_suites[suite.pk].append(child_case)
            roots.append(suite)
        for root in roots:
            parent_id = root.pk
    ...
    def test_suite_union_order_by_filter(
        ...
        with allure.step('Generate and sort plans'):
            expected_suites = []
            for idx in range(3):
                suite = model_to_dict_via_serializer(
                    test_suite_factory(project=project, parent=root_suite, name=str(idx)),
                    TestSuiteUnionMockSerializer,
                    refresh_instances=True,
                )
        ...
        response_data = authorized_superuser_client.send_request(
            ...
            query_params={
                'project': project.pk,
                'parent': root_suite.pk,
                'ordering': f'-{order_by}' if descending else order_by,
            },
        ).json_strip()
        with allure.step('Validate ordering'):
            assert expected_suites == response_data

    @allure.title('Test suite union search filter')
    def test_suite_union_search(
        self,
        test_suite_factory,
        test_case_factory,
        project,
        ...
        found_name = 'Cat plan'
        root_suite = test_suite_factory(project=project)
        found_suite = test_suite_factory(project=project, parent=root_suite, name=found_name)
        container_suite = test_suite_factory(project=project, parent=root_suite)
        test_suite_factory(project=project, parent=container_suite, name=found_name)
        expected_data = [
            ...
            model_to_dict_via_serializer(
                test_case_factory(project=project, suite=root_suite, name=found_name),
                TestCaseUnionMockSerializer,
                refresh_instances=True,
            ),
        ]
    ...
    @allure.title('Test labels filter on test suite union')
    @pytest.mark.parametrize(
    ...
    def test_union_suite_filter_by_labels(
        self,
        authorized_superuser_client,
        cases_with_labels,
        label_indexes,
        ...
        labels, _, _, _ = cases_with_labels
        query_params = {'parent': test_suite.id, 'project': project.id}
        if label_indexes:
        ...
        with allure.step('Move plans to deeper nesting to validate nested filtering works'):
            root_suite = test_suite_factory(project=project)
            test_suite.parent = root_suite
            test_suite.save()
        with allure.step('Create not found suite'):
            test_suite_factory(project=project, parent=root_suite)
        with allure.step('Send request after nesting data deeper'):
            query_params['parent'] = root_suite.pk
            content = authorized_superuser_client.send_request(
                self.view_name_union,
                query_params=query_params,
            ).json()
        with allure.step('Validate number of suites'):
            if number_of_items:
                assert content.get('count') == 1
            else:
                assert content.get('count') == 0

    @allure.title('Test search cases in suite')
    def test_search_cases_in_suite(
        self,
        authorized_superuser_client,
        project,
        test_suite_factory,
        ...
        name_for_search = 'find_me'
        case_ids = []
        for idx in range(constants.NUMBER_OF_OBJECTS_TO_CREATE):
            if idx % 2 == 0:
                case = test_case_factory(name=name_for_search + str(idx), suite=test_suite, project=project)
                case_ids.append(case.id)
            else:
                test_case_factory(suite=test_suite, project=project)
        response_content = authorized_superuser_client.send_request(
            self.view_name_cases,
            reverse_kwargs={'pk': test_suite.id},
            query_params={'search': name_for_search},
        ).json()
...
class TestSuiteEndpointsQueryParams:
    ...
    def test_suite_filter(self, superuser_client, generate_projects_and_suites):
        for project in generate_projects_and_suites:
            with allure.step(f'Validate suites for project {project.id}'):
                ...
                expected_dict = model_to_dict_via_serializer(suites, TestSuiteMockOutputSerializer, many=True)
                response = superuser_client.send_request(
                    self.view_name_list,
                ...
                with allure.step('Validate suites'):
                    assert actual_dict == expected_dict, f'{expected_dict}, {actual_dict}'

    @pytest.mark.django_db(reset_sequences=True)
    @allure.title('Test search')
    @pytest.mark.skip('Skipped because treeview is no longer viable need to rewrite')
    def test_search(self, superuser_client, test_suite_factory, project):
        ...
        actual_data = superuser_client.send_request(
            self.view_name_list,
            query_params={'project': project.id, 'treesearch': search_name, 'treeview': 1},
    ...
    @pytest.mark.skip('Decide should this be in 2.0 or not')
    ...
    def test_tree_search_and_search(
        ...
        search_name = 'search_name'
        parent_test_suite = test_suite_factory(project=project)
        child_test_suite = test_suite_factory(project=project, parent=parent_test_suite, name=search_name)
        if search_param == 'treesearch':
            query_params = {search_param: search_name, 'parent': 'null'}
            search_id = parent_test_suite.id
        else:
            query_params = {search_param: search_name}
            search_id = child_test_suite.id
    ...
    def test_search_by_path_exact(self, superuser_client, test_suite_factory, project, param_name):
        ...
        for _ in range(number_to_create):
            tmp_suite = test_suite_factory(project=project, parent=tmp_suite)
            suites.append(tmp_suite)
        for idx in range(len(suites)):
            expected_suites = [suites[idx]] if param_name == 'path_exact' else suites[idx:]
            expected_output = model_to_dict_via_serializer(
                expected_suites,
                TestSuiteMockOutputSerializer,
                many=True,
            ...
            path = '/'.join([suite.name for suite in suites[:idx + 1]])
            actual_suite_list = superuser_client.send_request(
                self.view_name_list,
                expected_status=HTTPStatus.OK,
                request_type=RequestType.GET,
                query_params={'project': project.pk, param_name: path},
            ).json()['results']
            with allure.step('Validate response body'):
                assert expected_output == actual_suite_list
