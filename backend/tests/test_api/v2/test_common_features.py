...
from tests.mock_serializers.v2 import TestCaseMockSerializer, TestMockSerializer
from testy.tests_description.api.v2.serializers import TestSuiteSerializer
...
class TestCommonFeatures:
    project_list_view = 'api:v2:project-list'
    plan_list_view = 'api:v2:testplan-list'
    test_list_view = 'api:v2:test-list'
    result_list_view = 'api:v2:testresult-list'

    @pytest.mark.parametrize(
        'factory_name, view_name, serializer_class, is_paginated', [
            ('test_case_factory', 'api:v2:testcase-list', TestCaseMockSerializer, True),
            ('test_suite_factory', 'api:v2:testsuite-list', TestSuiteSerializer, True),
            ('test_plan_factory', 'api:v2:testplan-list', TestPlanOutputSerializer, True),
            ('test_factory', 'api:v2:test-list', TestMockSerializer, True),
            ('parameter_factory', 'api:v2:parameter-list', ParameterSerializer, False),
            ('test_result_factory', 'api:v2:testresult-list', TestResultSerializer, False),
    ...
    def test_project_filter(
        ...
        for project, expected_instances in parent_to_expected_list:
            ...
            expected_dictionaries = (
                model_to_dict_via_serializer(expected_instances, serializer_class, many=True, refresh_instances=True)
    ...
    @pytest.mark.parametrize(
        ...
        [
            ('test_case_factory', ('suite', 'test_suite_factory'), 'api:v2:testcase-{0}', TestCaseMockSerializer, True),
            ('test_factory', ('plan', 'test_plan_factory'), 'api:v2:test-{0}', TestMockSerializer, True),
            ('test_result_factory', ('test', 'test_factory'), 'api:v2:testresult-{0}', TestResultSerializer, False),
        ],
    ...
    @pytest.mark.skip('No more treeview ')
    @pytest.mark.parametrize('is_archive_flag', [None, 0, 1], ids=['No flag', 'Negative flag', 'Positive flag'])
