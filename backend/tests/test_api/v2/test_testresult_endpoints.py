...
from testy.tests_representation.api.v2.serializers import TestResultSerializer
...
class TestResultEndpoints:
    view_name_list = 'api:v2:testresult-list'
    view_name_detail = 'api:v2:testresult-detail'
    project_view_name_detail = 'api:v2:project-detail'
    plan_view_name_detail = 'api:v2:testplan-detail'
    view_name_attributes = 'api:v2:testresult-attributes'
    ...
    def test_partial_update(self, api_client, authorized_superuser, test_result, user, result_status_factory):
        expected_status = result_status_factory(project=test_result.project)
        update_dict = {
            'user': user.id,
            'status': expected_status.pk,
            'comment': 'new_comment',
        }
        ...
        assert test_result.test.last_status == expected_status
    ...
    def test_soft_delete(
            self, api_client, authorized_superuser, test_result_factory, project, test_factory, test_plan_factory,
        ...
        for _ in range(constants.NUMBER_OF_OBJECTS_TO_CREATE):
            result = test_result_factory(test=test, project=project, status=result_status_factory(project=project))
        response = api_client.send_request(self.view_name_list, query_params={'project': project.id})
