...
class TestLabelModel:
    relation_name = Label._meta.label_lower.replace('.', '_')

    @pytest.mark.parametrize('parameter_name', ['project', 'name'])
    def test_not_null_constraint(self, parameter_name, label_factory):
        with pytest.raises(IntegrityError) as err:
            label_factory(**{parameter_name: None})
    ...
    def test_unique_together_constraint(self, label, label_factory, project):
        with pytest.raises(IntegrityError) as err:
