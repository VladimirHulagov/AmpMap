...
class TestSuiteBaseSerializer(serializers.ModelSerializer):
    url = HyperlinkedIdentityField(view_name='api:v2:testsuite-detail')
...
class TestSuiteUpdateOutputSerializer(TestSuiteBaseSerializer):
    parent = ParentMinSerializer(read_only=True)
...
class TestSuiteOutputSerializer(TestSuiteBaseSerializer):
    ...
    parent = ParentMinSerializer(read_only=True)
    total_cases_count = IntegerField(read_only=True)
    estimates = EstimateField(read_only=True)
    total_estimates = EstimateField(read_only=True)
...
class TestSuiteTreeBreadcrumbsSerializer(serializers.ModelSerializer):
    has_children = BooleanField(read_only=True)
    parent = ParentMinSerializer(read_only=True)
    ...
    def get_children(self, instance):
        ...
        return serializer_class(instance.child_test_suites.all(), many=True).data

    class Meta:
        model = TestSuite
        fields = ('id', 'name', 'has_children', 'parent', 'children')


class TestSuiteRootTreeBreadcrumbsSerializer(serializers.ModelSerializer):
    has_children = BooleanField(read_only=True)
    parent = ParentMinSerializer(read_only=True)
    children = SerializerMethodField()
...
class TestSuiteRetrieveSerializer(TestSuiteOutputSerializer):
    ...
    def get_breadcrumbs(cls, instance: TestSuite):
        ...
        return get_breadcrumbs_treeview(
            instances=tree,
            depth=len(tree) - 1,
            title_method=lambda suite: suite.name,
        )
...
class TestSuiteOutputCreateSerializer(TestSuiteSerializer):
    parent = ParentMinSerializer(read_only=True)


class TestCaseUnionSerializer(TestCaseListSerializer):
